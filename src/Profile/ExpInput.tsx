...
const ExpInput = (props: any) => {
    ...
    useEffect(() => {
        if (!props.add) form.setValues({
            title: props.title, company: props.company, location: props.location,
            description: props.description, startDate: new Date(props.startDate), endDate: new Date(props.endDate), working: props.working
        });
    }, []);
    ...
    const handleSave = () => {
        ...
        successNotification("Success", `Experiences ${props.add ? "Added" : "Updated"} successfully!`);
    }

    const [desc, setDesc] = useState(`${props.description}`);
    return <div className="flex flex-col gap-3">
        <div className="text-lg font-semibold">{props.add ? "Add New Experience " : "Edit Experience "}</div>
        <div className="flex gap-10 [&>*]:w-1/2 md-mx:gap-5  xs-mx:[&>*]:w-full xs-mx:flex-wrap my-3">
            <SelectInput form={form} name="title" {...select[0]} />
            <SelectInput form={form} name="company" {...select[1]} />
        </div>
        <SelectInput form={form} name="location" {...select[2]} />
        <Textarea {...form.getInputProps('description')} withAsterisk label="Summary" placeholder="Enter about summary.." autosize minRows={3}
        />
        <div className="flex gap-10 [&>*]:w-1/2 md-mx:gap-5  xs-mx:[&>*]:w-full xs-mx:flex-wrap my-3">
            ...
            <MonthPickerInput {...form.getInputProps("endDate")} disabled={form.getValues().working} withAsterisk minDate={form.getValues().startDate || undefined} maxDate={new Date()} label="Start Date" placeholder="End Date" />
        </div>
